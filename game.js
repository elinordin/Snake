...
function draw() {
  ...
  if (colliding) { //If snake's position is equal to the gameboard or tail
    ...
    if (score > h1score){ //If score is higher than leading score
      document.getElementById("new-highscore").style.display = "block"; //Display highscore window
      input.addEventListener("input", function(e){ //Listen for the user to type, the e in this case is what the user inputs
        h3name = h2name;
        h3score = h2score;
        h2name = h1name;
        h2score = h1score;
        h1name = e.target.value;
        h1score = score;
        //highscore.splice(0, 0, (new Highscore(e.target.value, score)));
    ...
    } else if (score > h2score){ //If score is higher than second score
      document.getElementById("new-highscore").style.display = "block";
      input.addEventListener("input", function(e){
        h3name = h2name;
        h3score = h2score;
        h2name = e.target.value;
        h2score = score;
        //highscore.splice(1, 0, (new Highscore(e.target.value, score)));
    ...
    } else if (score > h3score){ //If score is higher than third score THIS ONE WORKS
      document.getElementById("new-highscore").style.display = "block";
      input.addEventListener("input", function(e){ //Listen for the user to type, the e in this case is what the user inputs
        h3name = e.target.value;
        h3score = score;
        //highscore.splice(2, 0, (new Highscore(e.target.value, score))); //Position of new array item, how many are being removed, values to send to constructor function
        document.addEventListener("keydown", function(e){
          if (e.key == "Enter"){
            highscoreToGameOver();
          }
